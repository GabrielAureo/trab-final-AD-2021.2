...
def mm1_queue(lamda, mu, max_time = 1000, max_events = 1000):
  ...
  #fila FIFO real, recebe apenas arrivals e remove o primeiro item em um evento de serviços 
  queue = [time]
  ...
  while(nevents < max_events and time < max_time):
    ...
    if(etype == 'a'):
      ...
      waits.append(time - arrival_time)
      if(N > 0):
